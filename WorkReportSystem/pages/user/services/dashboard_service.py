...
def build_submit_pie_chart(real_name):
...
#最近7天提交趋势
def build_submit_line_chart(real_name):
...
#我的项目TOP5
def build_my_project_top5_html(real_name):
    ...
    cur.execute("""
        ...
        order by id desc
        limit 5
    """,(f"%{real_name}%",f"%{real_name}%",f"%{real_name}%",f"%{real_name}%",f"%{real_name}%"))

    rows = cur.fetchall()

    conn.close()

    html = "<div class='panel-card'><div class='title'>TOP5项目</div>"
...
#最近提交记录
def build_recent_submit_html(real_name):
    ...
    html = "<div class='panel-card'><div class='title'>最近提交</div>"
...
def load_dashboard_base(real_name):

    return (

        build_dashboard_kpi_html(
            real_name
        ),

        build_submit_pie_chart(
            real_name
        ),

        build_submit_line_chart(
            real_name
        ),
...
def build_dashboard_kpi_html(real_name):
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi-card blue">
            ...
            <div class="kpi-label">总提交</div>
        </div>

        <div class="kpi-card green">
            <div class="kpi-value">{report}</div>
            <div class="kpi-label">日报</div>
        </div>

        <div class="kpi-card orange">
            <div class="kpi-value">{meeting}</div>
            <div class="kpi-label">会议</div>
        </div>

        <div class="kpi-card purple">
            <div class="kpi-value">{project}</div>
